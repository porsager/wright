  ...
  function fetchResources() {
    chrome.send('Page.getResourceTree').then(result => {
      ...
      .filter(u => {
        ...
        return u.startsWith(config.url) && ext && ext !== '.css' && ext !== '.js'
      })
      .map(getPath)
      .filter(f => fs.existsSync(f))

      const add = newResources.filter(r => resources.indexOf(r) < 0)

      add.forEach(file => log.debug('Watching', path.relative(config.serve, file)))
  ...
  function run(filename) {
    ...
    chrome.send('Runtime.evaluate', {
      ...
      if (data.result.subtype === 'error')
        throw new Error(data.exceptionDetails)
    }).catch(error => {
