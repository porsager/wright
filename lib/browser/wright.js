(function () {
  ...
  function Ubre(ref) {
    ...
    var incoming = {
      ...
      publish: function (from, ref) {
          ...
          return subscriptions.has(name) && subscriptions.get(name).forEach(function (s) { return (
      ...
      request: function (from, ref) {
        ...
        if (!handlers.has(name))
          { return forward({ type: 'fail', id: id, body: new Error('NotFound') }, from) }

        tasks.set(id, from);
        Promise.resolve(handlers.get(name)(body, from))
        .then(function (body) {
          tasks.has(id) && (
            forward({ type: 'success', id: id, body: body === null ? undefined : body }, from),
            tasks.delete(id)
          );
        })
        .catch(function (body) {
          tasks.has(id) && (
            forward({ type: 'fail', id: id, body: body === null ? undefined : body }, from),
            tasks.delete(id)
          );
        });
      },
    ...
    ubre.publish = function (topic, body) {
      subscribers.has(topic) && subscribers.get(topic).forEach(function (s) { return forward({ type: 'publish', name: topic, body: body }, s); }
      );
    };
    ...
    ubre.subscribe = function(topic, body, fn) {
      ...
      open && forward({ type: 'subscribe', name: topic, body: body }, this.target);
      var subscription = { body: body, fn: fn, sent: open, target: this.target };
      subscriptions.add(topic, subscription);

      return {
        unsubscribe: function () {
          open && forward({ type: 'unsubscribe', name: topic, body: body }, this$1.target);
    ...
    ubre.request = function(name, body, id) {
      ...
      var promise = new Promise(function (resolve, reject) {
        cancel = function () { return (
          open && forward({ type: 'cancel',  id: id, body: body }, this$1.target),
          requests.delete(id),
          reject(new Error('cancelled'))
        ); };
        requests.set(id, { resolve: resolve, reject: reject, name: name, body: body, sent: open, target: this$1.target });
      });

      promise.cancel = cancel;

      open && forward({ type: 'request', id: id, name: name, body: body }, this.target);
      return promise
    };

    ubre.handle = function (name, fn) {
      typeof name === 'object'
        ? Object.keys(name).forEach(function (h) { return ubre.handle(h, name[h]); })
    ...
    ubre.open = function (target) {
      ...
      requests.forEach(function (r, id) { return !r.sent && (
        forward({ type: 'request', id: id, name: r.name, body: r.body }, r.target),
        r.sent = true
      ); });
    };
  ...
  var socket = new index('ws://' + location.host + '/wright')
      , ubre = Ubre.ws(socket);
  ...
  mithril.mount(div, { view: function () { return model.show && model.inspectRect && mithril('div' + bss(templateObject, model.inspectRect.left + model.inspectRect.width / 2, model.inspectRect.top + model.inspectRect.height / 2).$animate('0.3s', {
      ...
      mithril('span' + bss(templateObject$2, model.inspectRect.bottom + 8, model.inspectRect.left).$animate('0.3s', { from: bss(templateObject$3) }),
