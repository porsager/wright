...
utils.retryConnect = function(url, timeout) {
  return new Promise((resolve, reject) => {
    ...
    function connect() {
      utils.req(url).get(url, resolve).on('error', () => !timedOut && setTimeout(connect, 200))
    }
...
utils.wrightDataDirectory = function(process) {
  ...
  const wslPath =
    ...
    .reduce((acc, x) => acc.concat(x.PATH ? [x.PATH] : []), [])

  const wslUsername =
    wslPath
    .reduce((acc, x) => acc.concat(x.split(':')), [])
    .reduce((acc, x) => acc.concat(x.split('/mnt/c/Users').slice(1)), [])
    .reduce((acc, x) => acc.concat(x.split('/').slice(1, 2)), [])
