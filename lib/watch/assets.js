...
function waitAndReload(localPath) {
  if (fs.existsSync(localPath))
    return

  watch(localPath, file => {
    watch(localPath, () => {
      // Replace reload watcher with nothing
    })
...
function watchResource(src) {
  const ext = path.extname(src)
      ...
      , pathname = src && decodeURIComponent(url.parse(src).pathname)
  ...
  if (ext === '.js')
    return waitAndReload(localPath)

  if (!localPath || !isLocal || !ext || parsedSheet)
    return
