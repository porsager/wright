...
function handleInjectedScript(script, js) {
  watch(js.watch, file => {
    ...
    .then(code => {
      code = js.code = (js.injected ? utils.wrapInjectedCode(code) : code)
...
function replaceOnLoadScriptSource(js) {
  chrome.send('Page.addScriptToEvaluateOnLoad', {
    scriptSource: js.code
  }).then(data => {
    chrome.send('Page.removeScriptToEvaluateOnLoad', {
      identifier: js.injected
    })
    js.injected = data.identifier
  })
}
