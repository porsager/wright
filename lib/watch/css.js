...
module.exports = function() {
  ...
  chrome.on('CSS.styleSheetRemoved', styleSheetId => styles.delete(styleSheetId))
}
...
function handleFileCss(style, localPath) {
  fs.readFile(localPath, 'utf8', (err, content) => {
    ...
    style.text = content || ''
    styles.set(style.styleSheetId, style)
  })

  watch(localPath, file => {
    const notification = ui.notification('Injecting', path.relative(config.serve, file))
        , href = '[href^="' + path.relative(config.serve, file) + '"]'

    chrome.send('Runtime.evaluate', {
      expression: `document.querySelectorAll('link${ href }').forEach(el => {
        el.href = '${file + '?' + Date.now()}'
      })`
    })
