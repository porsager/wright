...
function handleFileCss(style, localPath) {
  ...
  watch(localPath, absolutePath => {
    const file = path.relative(config.serve, absolutePath)
        , notification = ui.notification('Injecting', path.relative(config.serve, file))
        , href = utils.slash(file)
...
function handleInjectedCss(style, css) {
  watch(css.watch, absolutePath => {
    const file = path.relative(config.serve, absolutePath)
        , notification = ui.notification('Injecting', file)
