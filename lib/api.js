  ...
  config.api = {
    ...
    inject: (script, scriptSource) => {
      ...
      chrome.replace(script, scriptSource, (err, result) => {
        if (err) {
          chrome.refresh()
          log('Failed hot reload ' + path.relative(config.cwd, script.scriptId) + ' refreshed instead')
        } else {
          chrome.run(config.reload)
          log('Hot reloaded ' + script.path ? path.relative(config.cwd, script.path) : 'injected script')
        }
      })
    },
    reload: (file) => {
      file = utils.slash(path.relative(config.cwd, file))
