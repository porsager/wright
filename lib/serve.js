...
function unpkg(content) {
  ...
  return content
    .replace(staticImportRegex, (_, a, b, c) => a + '/wright/modules/' + b + c)
    .replace(dynamicImportRegex, (_, a, b, c) => a + '/wright/modules/' + b + c)
}

function modules(req, res) {
  const importPath = req.url.slice(modulesPath.length)
      , pkgName = importPath.split('/')[0]
      , pkgPath = importPath.split('/').slice(1).join('/')
      , pkgBaseName = importPath.split(/[@/]/)[0]

  let pkgJson
  try {
    pkgJson = require.resolve(pkgBaseName + '/package.json', {
      paths: [config.serve, process.cwd()]
    })
  } catch (e) {
    // noop
  }

  fs.readFile(pkgJson, 'utf8')
    .then(JSON.parse)
    .then(x => pkgPath || x.module || x.browser || x.unpkg || x.main || 'index.js')
    .catch(() => pkgPath || 'index.js')
    .then(x =>
      fs.readFile(path.join(path.dirname(pkgJson), pkgPath + '.js'), 'utf8')
        .catch(() => fs.readFile(path.join(path.dirname(pkgJson), x), 'utf8'))
    )
    .then(x => {
      if (req.installed)
        log.debug('Installed and serving ' + pkgName)
      res.setHeader('Content-Type', 'text/javascript')
      res.end(unpkg(x))
    })
    .catch(x => {
      if (req.installed || req.url.endsWith('js.map')) {
        res.statusCode = 500
        res.end('Wright could not autoload ' + importPath)
        return
      }

      log.debug(pkgName + ' not found. Running npm install ' + pkgName)
      cp.exec('npm install ' + pkgName, { encoding: 'utf8' }, (err, stdout, stderr) => {
        req.installed = { err, stdout, stderr }
        modules(req, res)
      })
    })
}
