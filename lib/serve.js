...
function modules(req, res, corsy) {
  ...
  fs.readFile(path.join(pkgBase, 'package.json'), 'utf8').then(JSON.parse)
    ...
    .catch(x => {
      log.debug('Could not find module', pkgName, 'trying https://unpkg.com/' + pkgName + '?module')
      https.get('https://unpkg.com/' + pkgName + '?module', function OnResponse(response) {
        if (response.statusCode >= 300 && response.statusCode < 400)
          return https.get('https://unpkg.com' + response.headers.location, OnResponse)

        res.writeHead(response.statusCode, response.headers)
        response.pipe(res)

        if (response.statusCode >= 200 && response.statusCode < 300) {
          fs.ensureDirSync(pkgBase)
          response.pipe(fs.createWriteStream(path.join(pkgBase, 'index.js')))
        }
      })
    })
}
...
function sendHtml(res) {
  if (config.main.endsWith('.js'))
    return res.end(html(config.name) + scriptTag(config.main.replace(config.serve, ''), true))
