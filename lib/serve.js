  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (path.extname(req.url) === '.css') {
        ...
        if (css)
          return utils.promisify(css.compile).then(c => res.end(c)).catch(log.error)
      }
...
function handleJs(req, res) {
  ...
  if (js)
    return utils.promisify(js.compile).then(jail).then(code => res.end(code)).catch(log.error)
...
function addFiles(content) {
  ...
  return content.replace('html>', 'html>' + linkTags) + scriptTags
}
