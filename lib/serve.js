  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (path.extname(req.url) === '.css') {
        const sheet = config.css.find(f => f.path === req.url)
...
function handleJs(req, res) {
  const script = config.js.find(f => f.path === req.url)

  if (script)
    return resolved.then(script.compile).then(jail).then(code => res.end(code)).catch(log.error)
