  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (path.extname(req.url) === '.css') {
        const sheet = config.css.find(f => f.path === req.url.substr(1))

        if (sheet)
          return resolved.then(sheet.compile).then(c => res.end(c)).catch(log.error)
      }
...
function handleJs(req, res) {
  const script = config.js.find(f => f.path === req.url.substr(1))

  res.setHeader('content-type', 'text/javascript')

  if (script)
    return resolved.then(script.compile).then(jail).then(c => res.end(c)).catch(log.error)
...
function sendHtml(res) {
  if (!config.main.endsWith('.html'))
    return res.end(addFiles(html(path.basename(config.cwd))))
...
function addFiles(content) {
  const scriptTags = config.files.filter(f => path.extname(f) === '.js').map(scriptTag).join('')
      , linkTags = config.files.filter(f => path.extname(f) === '.css').map(linkTag).join('')

  return content.replace('</head>', linkTags + scriptTags + '</head>')
