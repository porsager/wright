...
const serve = new EventEmitter()
    , devFiles = { js: 1, css: 1, jpg: 1, png: 1, gif: 1 }
...
module.exports.start = function() {
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (config.external && !devFiles[path.extname(pathname).toLowerCase().slice(1)])
        return proxy.web(req, res)

      serveStatic(req, res, err => {
        serve.emit('get', path.join(config.serve, req.url))

        if (config.external && req.method === 'GET')
          hijack(res)
...
function hijack(res) {
  ...
  res.write = function(buf) {
    ...
    const body = buf.toString().toLowerCase()

    html = _statusCode === 200 && (body.startsWith('<!doctype html') || body.startsWith('<html'))
  ...
  res.end = function(a) {
    if (_statusCode && !res.headersSent)
      _writeHead.apply(res, _writeHeadArgs)
    if (hadFirstWrite)
      _write.call(res, html ? (injectedJs + injectedSocket) : '')
...
(function() {
  ...
  const socket = new WebSocket("ws://localhost:${port}/wright")
