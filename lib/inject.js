...
module.exports = function(chrome) {
  ...
  if (config.css) {
    chrome.on('Page.loadEventFired', (r) => {
      chrome.once('CSS.styleSheetAdded', s => styleSheetId = s.header.styleSheetId)
  ...
  function injectJs(source) {
    ...
    chrome.send('Debugger.setScriptSource', {
      ...
      if (config.run) {
        chrome.send('Runtime.evaluate', { expression: getScript(config.run) })
        log('Hot reloaded injected script')
      } else {
        chrome.send('Page.reload', { ignoreCache: true })
      }
    })
