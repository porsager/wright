...
module.exports = function() {
  ...
  return utils.request(config.chromeUrl + '/json/list/').then(allTabs => {
    ...
    utils.request(config.chromeUrl + '/json/list').then(closeNewTabs)
...
function closeNewTabs(tabs) {
  tabs.filter(t => t.url === 'chrome://newtab/').forEach(t => {
    utils.request(config.chromeUrl + '/json/close/' + t.id)
  })
}
